...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self.buttons = {
            ...
            "z_to_max": self._gtk.Button("home-z", _("Set Z to Max"), "color4"),
            "z_to_zero": self._gtk.Button("complete", _("Set Z to 0"), "color4"),
        }
        ...
        self.buttons["z-"].connect("clicked", self.move, "Z", "-")
        script = {"script": "SET_Z_ZERO_POSITION"}
        self.buttons["z_to_zero"].connect(
            "clicked",
            self._screen._confirm_send_action,
            _("Set Z axis position to 0?"),
            "printer.gcode.script",
            script,
        )
        script = {"script": "SET_Z_MAX_POSITION"}
        self.buttons["z_to_max"].connect(
            "clicked",
            self._screen._confirm_send_action,
            _("Set Z axis position to maximum?"),
            "printer.gcode.script",
            script,
        )
        ...
        grid.attach(self.buttons["z_to_zero"], 0, 0, 1, 1)
        grid.attach(self.buttons["z_to_max"], 2, 0, 1, 1)
