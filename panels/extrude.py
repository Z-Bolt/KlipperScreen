...
class ExtrudePanel(ScreenPanel):
    ...
    def __init__(self, screen, title, back=True):
        ...
        self.labels['load'].connect("clicked",self.confirm_load_unload,_("Are you sure want to load a NEW reel of filament (the rod pulling distance is 2000mm!)?"), "+")
        
        self.labels['unload'] = self._gtk.ButtonImage("arrow-up", _("Unload"), "color2")
        self.labels['unload'].connect("clicked",self.confirm_load_unload,_("Are you sure want to unload the current filament spool (the rod pulling distance is 2000mm!)?"),"-")
        ...
        for extruder in self._printer.get_tools():
            ...
            if i < limit:
                ...
                i += 1
        if i < (limit - 1):
            # extgrid.attach(self.labels['nozzle'], i + 1, 0, 1, 1)
