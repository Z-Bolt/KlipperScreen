...
class MainPanel(MenuPanel):
    ...
    def initialize(self, panel_name, items, extrudercount):
        ...
        for h in add_heaters:
            ...
            elif h.startswith("heater_generic"):
                self.labels[h] = self._gtk.ButtonImage("heater", self._gtk.formatTemperatureString(0, 0))
                self.labels[h].connect("clicked", self.menu_item_clicked, "temperature", {
                "name":  _('Temperature'),
                "panel": "temperature"
                })
        ...
        grid.attach(self.arrangeMenuItems(items, 3, True), 0, 0, 1, 1)
