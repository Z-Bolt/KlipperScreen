...
class KlipperScreenConfig:
    ...
    def __init__(self, configfile, screen=None):
        ...
        if self.get_main_config_option("select_mode", False):
            self._read_expert_config()
    ...
    def _read_expert_config(self):
        expert_config = configparser.ConfigParser()
        try:
            expert_config.read(self.default_expert_config_path)
            for i in ['menu __main', 'menu __print']:
                for j in expert_config.sections():
                    if j.startswith(i):
                        for k in list(self.config.sections()):
                            if k.startswith(i):
                                del self.config[k]
                        break
            self.config.read(self.default_expert_config_path)
        except KeyError:
            raise ConfigError(f"Error reading config: {self.default_expert_config_path}")
        except Exception:
            logging.exception("Unknown error with config")
